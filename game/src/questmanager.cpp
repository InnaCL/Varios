...
namespace quest
{
	...
	bool CQuestManager::CompileQuest(const string& quest)
	{
		char buf[256];
		snprintf(buf, sizeof(buf), "%s/%s", g_stQuestDir.c_str(), quest.c_str());

		int result = lua_dofile(L, buf);
		if (result != 0)
		{
			sys_err("LOAD_DO_QUEST_ERROR(%s)", buf);
			return false;
		}
		return true;
	}
