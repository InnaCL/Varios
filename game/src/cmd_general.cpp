...
ACMD(do_pkmode)
{
	...
	//const char* sz = "block_pk";
//	int check = ch->GetQuestFlag(sz);
	/*
	if(check){
		ch->ChatPacket(CHAT_TYPE_INFO, " esci %d",check);
		return;
	}
	ch->ChatPacket(CHAT_TYPE_INFO, " stai %d",check);
	*/
...
ACMD(do_info)
{
	...
	char szQuery[1024+1];
	snprintf(szQuery, sizeof(szQuery), "SELECT job, playtime, hp, level, alignment,last_play FROM player%s WHERE name='%s'", get_table_postfix(),arg1);
	std::auto_ptr<SQLMsg> msg(DBManager::instance().DirectQuery(szQuery));
	if (msg->Get()->uiNumRows == 0)
		return;
	MYSQL_ROW row = mysql_fetch_row(msg->Get()->pSQLResult);
	snprintf(buf, sizeof(buf), "Job:%s # Playtime:%s # Hp:%s # Level:%s # Karma:%s # Last:%s # ",
