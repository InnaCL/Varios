...
namespace quest 
{
	...
	int pc_can_attack(lua_State * L){
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		ch->m_canAttack = true;
		return 1;
	}
	int pc_cannot_attack(lua_State * L){
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		ch->m_canAttack = false;
		return 1;
	}
	int pc_set_pk_mode(lua_State * L){
		if (!lua_isnumber(L, 1))
		{
	...
	*******************************************/
	...
	// {
		...
		// if (!lua_isnumber(L, 1))
		// {
			// sys_err("[%s]: Argument error in pc_set_level.", ch->GetName());
			// lua_pushboolean(L, false);
			// return 1;
		// }
	...
	{
		long gold = (long)lua_tonumber(L, -1);
