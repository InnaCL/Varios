...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	else if (pkVictim->IsPC())
	{
		...
		switch (pkVictim->GetJob())
		{
		 ...
		 switch (pkVictim->GetEmpire())
		 {
			 case 1://SHINSOO
			 iAtk += (iAtk * pkAttacker->GetPoint(POINT_ATTBONUS_SHINSOO)) / 100;
			 break;

			 case 2://CHUNJO
				 iAtk += (iAtk * pkAttacker->GetPoint(POINT_ATTBONUS_CHUNJO)) / 100;
			     break;

			 case 3://JINNO
			      iAtk += (iAtk * pkAttacker->GetPoint(POINT_ATTBONUS_JINNO)) / 100;
				  break;
			}
