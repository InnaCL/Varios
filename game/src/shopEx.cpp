...
int CShopEx::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	long long dwPrice = (long long)r_item.price;
	...
	case SHOP_COIN_TYPE_SECONDARY_COIN:
		{
			long long count = (long long)ch->CountSpecifyTypeItem(ITEM_SECONDARY_COIN);
			if (( long long)count < dwPrice)
			{
				sys_log(1, "ShopEx::Buy : Not enough myeongdojun : %s has %lld, price %lld", ch->GetName(), count, dwPrice);
