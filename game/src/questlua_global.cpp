...
namespace quest
{
	...
	int _add_goto_info(lua_State* L)
	{
		...
		if (lua_isnumber(L, 6))
			money = (int)lua_tonumber(L, 6);
		if (lua_isnumber(L, 7))
			level = (int)lua_tonumber(L, 7);
		if (!name) 
			return 0;

		CHARACTER_AddGotoInfo(name, empire, mapIndex, x, y,money,level);
		return 0;
	}
	...
	struct CanNotAttack
	{
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC())
					ch->m_canAttack = false;
				
			}
		}
	};
	int _set_all_can_attack(lua_State *L){
		if (!lua_isnumber(L, 1))
		{
			sys_err("invalid argument");
			return 0;
		...
		LPSECTREE_MAP pkMap = SECTREE_MANAGER::instance().GetMap(index);
		if (pkMap == NULL) {
			sys_err("Set All can attack: SECTREE_MAP not found for #%d", index);
			return 0;
		}
		CanAttack f;
		pkMap->for_each(f);
	...
	int _set_all_cannot_attack(lua_State *L){
		if (!lua_isnumber(L, 1))
		{
			sys_err("invalid argument");
			return 0;
		...
		LPSECTREE_MAP pkMap = SECTREE_MANAGER::instance().GetMap(index);
		if (pkMap == NULL) {
			sys_err("Set All can not attack: SECTREE_MAP not found for #%d", index);
			return 0;
		}
		CanNotAttack f;
		pkMap->for_each(f);
	...
	-- local res1, res2 = mysql_direct_query("select * from player.player limit 2;")
	...
	*/
	...
	void RegisterGlobalFunctionTable(lua_State* L)
	{
		...
		luaL_reg global_functions[] =
		{
			...
			{ "set_all_cannot_attack", _set_all_cannot_attack },
			{ "set_all_can_attack", _set_all_can_attack },
