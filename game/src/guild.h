...
class CGuild
{
	public:
		...
		// Guild Money
		void		RequestDepositMoney(LPCHARACTER ch, long long iGold);
		void		RequestWithdrawMoney(LPCHARACTER ch, int  iGold);

		void		RecvMoneyChange(long long iGold);
		void		RecvWithdrawMoneyGive(long long iChangeGold); // bGive==1 이면 길드장에게 주는 걸 시도하고 성공실패를 디비에게 보낸다
