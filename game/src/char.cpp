...
void CHARACTER::UpdatePacket()
{
	...
	//sys_log(0, "m_bPKMode %d",pack.bPKMode);
...
//					AutoGiveItem(27003, 2);
		...
		case POINT_GOLD:
			{
				...
				if (LLONG_MAX <= nTotalMoney)
				{
					sys_err("[OVERFLOW_GOLD] Overflow (LLONG_MAX) id %u name %s", GetPlayerID(), GetName());
		...
		case POINT_ATTBONUS_SHINSOO:
		case POINT_ATTBONUS_CHUNJO:
		case POINT_ATTBONUS_JINNO:
			SetPoint(type, GetPoint(type) + amount);
			val = GetPoint(type);
			break;
...
void CHARACTER::SetPetRider(LPCHARACTER ch)
{
	m_petchRider = ch;
}

LPCHARACTER CHARACTER::GetPetRider() const
{
	return m_petchRider;
}
