...
void CHARACTER_AddGotoInfo(const std::string& c_st_name, BYTE empire, int mapIndex, DWORD x, DWORD y,int money,int level )
{
	...
	newGotoInfo.x = x;
	newGotoInfo.y = y;
	newGotoInfo.money = money;
	newGotoInfo.level = level;
...
bool CHARACTER_GoToName(LPCHARACTER ch, BYTE empire, int mapIndex, const char* gotoName)
{
	...
	for (i = gs_vec_gotoInfo.begin(); i != gs_vec_gotoInfo.end(); ++i)
	{
		...
		if (c_eachGotoInfo.empire == 0 || c_eachGotoInfo.empire == empire)
		{
			...
			int gold = c_eachGotoInfo.money;
			int level = c_eachGotoInfo.level;
			if(gold && level){
				
				if(ch->GetLevel() < level || ch->GetGold() < gold){
					ch->ChatPacket(CHAT_TYPE_INFO,"Non hai abbastanza soldi o il tuo livello è troppo basso");
					return false;
				}
...
ACMD(do_warp_gui)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Warp gui:< parameter error>");
		return;
	}
	int mapIndex = 0;
	BYTE empire = 0;
...
ACMD(do_state)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "%s", buf);
	ch->ChatPacket(CHAT_TYPE_INFO, "GOLD %lld", tch->GetGold());
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "   SHINSOO:%3d%% CHUNJO:%3d%%  JINNO:%3d%%",
			tch->GetPoint(POINT_ATTBONUS_SHINSOO),
			tch->GetPoint(POINT_ATTBONUS_CHUNJO),
			tch->GetPoint(POINT_ATTBONUS_JINNO));
		...
		case 5: // max_sp
			{
				int amount = 0;
				str_to_number(amount, arg3);
				tch->PointChange(POINT_ATTBONUS_JINNO, amount, true);
				tch->PointChange(POINT_ATTBONUS_SHINSOO, amount, true);
				tch->PointChange(POINT_ATTBONUS_CHUNJO, amount, true);
...
ACMD(do_jumpguild)
{
	LPDUNGEON pDungeon = CDungeonManager::instance().Create(ch->GetMapIndex());
	pDungeon->JumpGuild(ch->GetGuild(),ch->GetMapIndex() ,  ch->GetX()/100, ch->GetY()/100);
}
ACMD(do_makeguild)
{
	if (ch->GetGuild())
...
ACMD(do_private)
{
	char arg1[256];
	one_argument(argument, arg1, sizeof(arg1));
	ch->ChatPacket(CHAT_TYPE_INFO, "SDSDSDSDSD");
	if (!*arg1)
	{
...
ACMD(do_qc){
	...
	check = quest::CQuestManager::instance().CompileQuest(arg);
	ch->ChatPacket(CHAT_TYPE_INFO, "compiling %s ", arg.c_str());
	if (check){
		ch->ChatPacket(CHAT_TYPE_INFO, "%s compiled", arg.c_str());
	}
	else 
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "%s usage: qc <name.quest> or qc <name>", arg.c_str());
	}
}
ACMD(do_reload)
{
...
ACMD(do_ticket){
		...
		snprintf(szQuery, sizeof(szQuery),
			"SELECT mName FROM common.gmlist");
		...
		{
			ch->ChatPacket(0, (char*)row[0]);



			//if (!stAuth.compare("IMPLEMENTOR") || !stAuth.compare("GOD") || !stAuth.compare("HIGH_WIZARD") || !stAuth.compare("LOW_WIZARD") || !stAuth.compare("WIZARD"))
		}
...
ACMD(do_stat_plus_amount)
{
	...
	{
		case POINT_HT : // Ã¼·Â
			if (nPoint + ch->GetPoint(POINT_HT) > MAX_STATUS)
			{
		...
		case POINT_IQ : // Áö´É
			if (nPoint + ch->GetPoint(POINT_IQ) > MAX_STATUS)
			{
		...
		case POINT_ST : // ±Ù·Â
			if (nPoint + ch->GetPoint(POINT_ST) > MAX_STATUS)
			{
		...
		case POINT_DX : // ¹ÎÃ¸
			if (nPoint + ch->GetPoint(POINT_DX) > MAX_STATUS)
			{
