...
namespace quest
{
	...
	int affect_add_collect_pet(lua_State * L)
	{
		if (!lua_isnumber(L, 1) )
		{
			sys_err("invalid 1 argument");
			return 0;
		}
		if (!lua_isnumber(L, 2))
		{
			sys_err("invalid 2 argument");
			return 0;
		}
		if (!lua_isnumber(L, 1))
		{
			sys_err("invalid 3winscp argument");
			return 0;
		}
		...
		if (ch->FindAffect(AFFECT_PET, applyOn)) // 퀘스트로 인해 같은 곳에 효과가 걸려있으면 스킵
			return 0;
	...
	int affect_remove_collect_pet(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	int affect_remove_all_pet(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	void RegisterAffectFunctionTable()
	{
		luaL_reg affect_functions[] =
		{
			...
			{ "remove_all_pet", affect_remove_all_pet },
			{ "remove_all_collect", affect_remove_all_collect },
			{ "add_collect_pet", affect_add_collect_pet },
